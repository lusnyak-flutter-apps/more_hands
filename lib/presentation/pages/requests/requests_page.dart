...
class RequestsPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider<RequestsCubit>(
      create: (BuildContext context) => getIt<RequestsCubit>()
        ..getUnseenCounts()
        ..getRequests(),
...
class _RequestsView extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return BlocBuilder<RequestsCubit, RequestsState>(builder: (context, state) {
      final cubit = context.read<RequestsCubit>();
      final (filter, section, requests) =
          (state.selectedStatus, state.selectedType, state.requests);
        ...
        bottomSheet: MHBottomTabbar(
          ...
          items: [
            MHBottomNavigationBarItem(
              ...
              badgeValue: state.receiverUnseenCount,
              index: 0,
            ),
            MHBottomNavigationBarItem(
              ...
              icon: MoreHandsAssets.icons.chat.svg(height: 20.r),
              activeIcon: MoreHandsAssets.icons.chatYellow.svg(height: 20.r),
              badgeValue: state.senderUnseenCount,
              index: 1,
            ),
          ],
