...
class RequestsCubit extends Cubit<RequestsState> {
  ...
  Future<void> getRequests() async {
    ...
    try {
      final requests = await getIt<RequestsRepository>().getRequests(
          status: state.selectedStatus, type: state.selectedType,
          from: state.requests.length, to: state.requests.length + 10
      );
  ...
  Future<void> changeSection(int section) async {
    ...
    emit(state.copyWith(selectedType: type, selectedStatus: RequestStatus.all, requests: <RequestModel>[]));
    await getRequests();
  }
