  ...
  Future<void> sendRequest() async {
    if(state.receiverId != null && controller.text.isNotEmpty) {
      ...
      final param = SendRequestModel(
          receiverId: state.receiverId, rqText: controller.text);
      debugPrint(param.toJson().toString());
      try {
        await getIt<RequestsRepository>()
            .createAndSendRequest(sendModel: param) ;
        emit(state.copyWith(loading: false, completed: true, haveNoSubscription: false));
      } catch (e) {
        emit(state.copyWith(loading: false, completed: false, haveNoSubscription: true));
      }
    }
  }
}
